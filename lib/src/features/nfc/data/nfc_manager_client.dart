...
class NfcManagerClient implements NfcOperations {
  NfcManagerClient._() : _manager = _tryGetDefaultManager(), _settingsLauncher = AndroidIntentNfcSystemSettingsLauncher();
  ...
  Future<NfcTagSnapshot> scanSingleTag({required Duration timeout}) async {
    final manager = _manager;
    if (manager == null) {
      throw const NfcException(code: NfcErrorCode.unsupported, message: 'NFC is not supported on this platform/device.');
    }
