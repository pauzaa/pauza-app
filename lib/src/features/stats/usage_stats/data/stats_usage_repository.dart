...
class StatsUsageRepositoryImpl implements StatsUsageRepository {
  StatsUsageRepositoryImpl({required UsageStatsManager usageStatsManager, required InstalledAppsManager installedAppsManager})
  ...
  Future<IList<UsageStats>> getUsageStats({required DateTime start, required DateTime end}) async {
    ...
    return usage.where((stat) => stat.totalDuration > Duration.zero).toIList().sort((a, b) => b.totalDuration.compareTo(a.totalDuration));
  }
