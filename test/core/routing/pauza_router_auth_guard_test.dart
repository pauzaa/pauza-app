...
void main() {
  group('router guard flow', () {
    test('unauthenticated + [home] redirects to [auth]', () {
      final result = _applyGuards(isAuthenticated: false, isReady: true, state: <Page<Object?>>[PauzaRoutes.home.page()]);

      expect(_routesOf(result), <PauzaRoutes>[PauzaRoutes.auth]);
    });

    test('unauthenticated + [permissions] redirects to [auth]', () {
      final result = _applyGuards(isAuthenticated: false, isReady: false, state: <Page<Object?>>[PauzaRoutes.permissions.page()]);
    ...
    test('authenticated + [auth] redirects to [root]', () {
      final result = _applyGuards(isAuthenticated: true, isReady: true, state: <Page<Object?>>[PauzaRoutes.auth.page()]);
    ...
    test('authenticated + [profile] becomes [root, profile]', () {
      final result = _applyGuards(isAuthenticated: true, isReady: true, state: <Page<Object?>>[PauzaRoutes.profile.page()]);
    ...
    test('unauthenticated ignores permissions state and keeps auth-only flow', () {
      final result = _applyGuards(isAuthenticated: false, isReady: false, state: <Page<Object?>>[PauzaRoutes.auth.page()]);
