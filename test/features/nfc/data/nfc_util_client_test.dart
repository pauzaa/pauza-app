...
void main() {
  group('NfcUtilClient', () {
    ...
    test('checkAvailability returns available when platform is available', () async {
      final client = NfcUtilClient(manager: _FakeNfcManagerGateway(), settingsLauncher: const NoopNfcSystemSettingsLauncher());
    ...
    test('scanSingleTag maps session user cancel to cancelled exception', () async {
      ...
      await expectLater(pending, throwsA(isA<NfcException>().having((exception) => exception.code, 'code', NfcErrorCode.cancelled)));
    });
