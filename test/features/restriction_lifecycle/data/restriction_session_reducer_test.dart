  ...
  group('RestrictionSessionReducer', () {
    ...
    test('START -> PAUSE -> RESUME -> END derives counters and end time', () {
      ...
      final afterStart = reducer.reduce(
        event: _event(id: 'e1', sessionId: sessionId, action: RestrictionLifecycleAction.start, occurredAtEpochMs: 1_000),
        currentSession: null,
        nowUtc: now,
      );

      final afterPause = reducer.reduce(
        event: _event(id: 'e2', sessionId: sessionId, action: RestrictionLifecycleAction.pause, occurredAtEpochMs: 2_000),
        currentSession: afterStart,
        nowUtc: now.add(const Duration(milliseconds: 1)),
      );

      final afterResume = reducer.reduce(
        event: _event(id: 'e3', sessionId: sessionId, action: RestrictionLifecycleAction.resume, occurredAtEpochMs: 3_500),
    ...
    test('END while paused closes pause window at end timestamp', () {
      ...
      final afterStart = reducer.reduce(
        event: _event(id: 'e10', sessionId: sessionId, action: RestrictionLifecycleAction.start, occurredAtEpochMs: 1_000),
        currentSession: null,
        nowUtc: now,
      );

      final afterPause = reducer.reduce(
        event: _event(id: 'e11', sessionId: sessionId, action: RestrictionLifecycleAction.pause, occurredAtEpochMs: 2_000),
        currentSession: afterStart,
        nowUtc: now.add(const Duration(milliseconds: 1)),
      );

      final afterEnd = reducer.reduce(
        event: _event(id: 'e12', sessionId: sessionId, action: RestrictionLifecycleAction.end, occurredAtEpochMs: 5_000),
        currentSession: afterPause,
        nowUtc: now.add(const Duration(milliseconds: 2)),
      );
    ...
    test('duplicate START on same session marks anomaly', () {
      ...
      final firstStart = reducer.reduce(
        event: _event(id: 'e30', sessionId: sessionId, action: RestrictionLifecycleAction.start, occurredAtEpochMs: 1_000),
        currentSession: null,
        nowUtc: now,
      );

      final duplicateStart = reducer.reduce(
        event: _event(id: 'e31', sessionId: sessionId, action: RestrictionLifecycleAction.start, occurredAtEpochMs: 1_200),
