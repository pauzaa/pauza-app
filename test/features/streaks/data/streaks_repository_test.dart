...
void main() {
  group('StreaksRepositoryImpl', () {
    test(
      'excludes anomalous sessions from rollups and qualification',
      () async {
        final database = _FakeLocalDatabase();
        final nowUtc = DateTime.utc(2026, 1, 15, 12);
        final repository = StreaksRepositoryImpl(
        ...
        database.seedEvents(
          ...
          events: [
            _event(
              action: 'START',
              occurredAtUtc: nowUtc.subtract(const Duration(minutes: 30)),
            ),
    ...
    test(
      'incremental refresh no-op on unchanged cursor and reprocess on updated session',
      () async {
        final database = _FakeLocalDatabase();
        final nowUtc = DateTime.utc(2026, 1, 15, 12);
        final repository = StreaksRepositoryImpl(
        ...
        expect(
          database.aggregateUpsertCount,
          greaterThan(firstAggregateUpserts),
        );
    ...
    test('dedupes in-flight refreshes', () async {
      final database = _FakeLocalDatabase()
        ..restrictionSessionsQueryDelay = const Duration(milliseconds: 30);
      final nowUtc = DateTime.utc(2026, 1, 15, 12);
      final repository = StreaksRepositoryImpl(
        localDatabase: database,
        nowUtc: () => nowUtc,
    ...
    test(
      'getGlobalSnapshot includes in-progress contribution for today',
      () async {
        final nowUtc = DateTime.utc(2026, 1, 15, 12);
        ...
        database.seedEvents(
          ...
          events: [
            _event(
              action: 'START',
              occurredAtUtc: nowUtc.subtract(const Duration(minutes: 12)),
            ),
          ],
        );
        ...
        expect(
          snapshot.targetDurationPerDay,
          StreakConstants.targetDurationPerDay,
        );
      },
    );
...
final class _FakeLocalDatabase implements LocalDatabase {
  final Map<String, _RestrictionSessionRow> restrictionSessions =
      <String, _RestrictionSessionRow>{};
  final Map<String, List<_DateTimeEvent>> restrictionEvents =
      <String, List<_DateTimeEvent>>{};
  final Map<String, _StreakRollupRow> streakSessionDailyRollups =
      <String, _StreakRollupRow>{};
  final Map<String, _StreakDailyAggregateRow> streakDailyAggregates =
      <String, _StreakDailyAggregateRow>{};
  ...
  void seedEvents({
    required String sessionId,
    required List<_DateTimeEvent> events,
  }) {
    restrictionEvents[sessionId] = events;
  }

  void seedDailyAggregate({
    ...
    if (sql.contains('FROM streak_daily_aggregates')) {
      ...
      final rows =
          streakDailyAggregates.values
            ...
            ..sort(
              (a, b) => (a['local_day'] as String).compareTo(
                b['local_day'] as String,
              ),
            );
      return rows;
    }
...
final class _FakeTransaction implements Transaction {
  _FakeTransaction({required _FakeLocalDatabase fakeDatabase})
  ...
  Future<List<Map<String, Object?>>> rawQuery(
          ...
          _fakeDatabase.restrictionSessions.values
              .where(
                (row) =>
                    ...
                    (row.updatedAtEpochMs == cursorUpdatedAt &&
                        row.sessionId.compareTo(cursorSessionId) > 0),
              )
              .toList(growable: false)
            ..sort((a, b) {
              final byUpdated = a.updatedAtEpochMs.compareTo(
                b.updatedAtEpochMs,
              );
              if (byUpdated != 0) {
                return byUpdated;
    ...
    if (sql.contains(
      'FROM streak_session_daily_rollups WHERE session_id = ?',
    )) {
    ...
    if (sql.contains('FROM restriction_lifecycle_events')) {
      final sessionId = arguments!.first as String;
      final events =
          _fakeDatabase.restrictionEvents[sessionId] ??
          const <_DateTimeEvent>[];
    ...
    if (sql.contains('FROM streak_session_daily_rollups') &&
        sql.contains('COALESCE(SUM(effective_ms), 0)')) {
      final localDay = arguments!.first as String;
      final dayRows = _fakeDatabase.streakSessionDailyRollups.values.where(
  ...
  Future<int> rawUpdate(String sql, [List<Object?>? arguments]) async {
    ...
    if (sql.contains('UPDATE streak_rollup_state') &&
        sql.contains('session_cursor_updated_at')) {
      _fakeDatabase.streakRollupState = <String, Object?>{
  ...
  Future<int> rawDelete(String sql, [List<Object?>? arguments]) async {
    ...
    if (sql.contains(
      ...
      final localDay = args[0] as String;
      return _fakeDatabase.streakDailyAggregates.remove(localDay) == null
          ? 0
          : 1;
    }
